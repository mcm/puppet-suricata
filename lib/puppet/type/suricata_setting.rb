Puppet::Type.newtype(:suricata_setting) do
  ...
  desc <<-EOT
    ...
    Example:
      ...
      suricata_setting { 'simple_example':
        target => '/etc/example.yaml',
        key    => 'greeting',
        value  => 'hello',
      }

      suricata_setting { 'nested_key_example1':
        target  => '/etc/example.yaml',
        key     => 'database/username',
        value   => 'console',
      }

      suricata_setting { 'nested_key_example2':
        target  => '/etc/example.yaml',
        key     => 'database/password',
        value   => 'passw0rd',
      }

      suricata_setting { 'nested_key_example3':
        target  => '/etc/example.yaml',
        key     => 'one/two/three',
        type    => 'array',
        value   => [ 'a', 'b', 'c' ],
    ...
    In this example, several suricata_settings were specified and the resulting
    merged hash was created in /etc/example.yaml. If /etc/example.yaml had
    already contained data, any keys not specified in a suricata_setting resource
    would be preserved.
...
#      @resource[:nodisplay] ? "[new value redacted]" : display
      display = if new_value.is_a?(Array) and new_value.size > 1
        new_value
      else
        new_value.join(' ')
      end
